...
def get_parameter_groups(model, learning_rate, weight_decay=1e-5, bone_lr_scale=0.01, fix_steps = 5, constant_cls = False, constant_pos = False, skip_list=()):
    ...
    gammas = ['blocks.{}.coreset_selection_layer.gamma'.format(i) for i in range(len(model.blocks))]
    bettas = ['blocks.{}.coreset_selection_layer.beta'.format(i) for i in range(len(model.blocks))]

    # q_exp = ['blocks.{}.coreset_selection_layer.q_exp'.format(i) for i in range(len(model.blocks))]
    # sim_exp = ['blocks.{}.coreset_selection_layer.sim_exp'.format(i) for i in range(len(model.blocks))]
    # taus = ['blocks.{}.coreset_selection_layer.tau'.format(i) for i in range(len(model.blocks))]

    new_module_names = new_module_names + gammas + bettas

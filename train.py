...
def _in_debugger() -> bool:
    ...
    return bool(gettrace and gettrace())  # works for pdb / pydevd / debugpy


# Optional: honour an env-flag so you can force-disable from the shell
IN_DEBUG = _in_debugger() or os.getenv("WANDB_OFF") == "1"

if IN_DEBUG:
    os.environ["WANDB_MODE"] = "disabled"  # kills all network/CLI traffic
    os.environ["WANDB_SILENT"] = "true"  # hides the “wandb disabled” banner
...
def is_debugging():
    gettrace = getattr(sys, 'gettrace', None)
    if gettrace is None:
        return False  # No debugging function
    elif gettrace():
        return True  # Debugger is active
    else:
...
def main(args):
    ...
    if "dyvit" in args.model:
        if args.no_dyvit_teacher:
            ...
            if args.dataset.lower() != "imagenet":
                ...
                assert (os.path.isfile(args.dyvit_teacher_weights)), "Invalid DyViT Teacher Weight path: {}".format(
                    args.dyvit_teacher_weights)
    ...
    if "dyvit" in args.model:
        criterion = DynamicViTDistillationLoss(
            criterion, teacher_model, args.ratio_weight, args.cls_distill_weight, args.token_distill_weight,
            args.cls_weight, args.mse_token)
    else:
    ...
    if args.resume:
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            # gil - override lr schedular checkpoint state

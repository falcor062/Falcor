...
def train_one_epoch(model: torch.nn.Module, criterion,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        keys = ['gamma_layer_{}'.format(i) for i in range(len(model.blocks))]
        gamma_dic = {}
        if hasattr(model.blocks[0], 'coreset_selection_layer'):
            values = [model.blocks[i].coreset_selection_layer.gamma.detach().cpu().numpy() for i in
                      range(len(model.blocks))]
            gamma_dic = {key: value for key, value in zip(keys, values)}

        keys = ['beta_layer_{}'.format(i) for i in range(len(model.blocks))]
        beta_dic = {}
        if hasattr(model.blocks[0], 'coreset_selection_layer'):
            values = [model.blocks[i].coreset_selection_layer.beta.detach().cpu().numpy() for i in
                      range(len(model.blocks))]
            beta_dic = {key: value for key, value in zip(keys, values)}

        # keys = ['tau_layer_{}'.format(i) for i in range(len(model.blocks))]
        # tau_dic = {}
        # if hasattr(model.blocks[0], 'coreset_selection_layer'):
        #     values = [model.blocks[i].coreset_selection_layer.tau.detach().cpu().numpy() for i in
        #               range(len(model.blocks))]
        #     tau_dic = {key: value for key, value in zip(keys, values)}

        gamma_dic = beta_dic | gamma_dic
        if utils.is_main_process():
            if opt_step and (total_step % print_freq == 0 or epoch_step == len(data_loader)):
                log_dict = {"train_loss_step": loss_value,
                            **{f"lr-{lr_idx}": optimizer.param_groups[lr_idx]["lr"] for lr_idx in
                               range(len(optimizer.param_groups))},
                            "epoch": epoch + 1}
                log_dict.update(gamma_dic)
